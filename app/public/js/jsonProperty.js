...
class JsonProperty {
     ...
     */
    setChangeEventForNumber(object, id, prop) {
        id = `#${id}`;
        this.events[id] = 'change';
        $(document).on(this.events[id], id, (eventObject) => {
            const element = $(eventObject.target);
            const v = element.val().trim();
            if (v.match(/^\-?\d+$/)) {
     ...
     * @param prop property config
     */
    setKeyupEventForString(object, id, prop) {
        id = `#${id}`;
     ...
     * @param prop property config
     */
    setChangeEventForBoolean(object, id, prop) {
        id = `#${id}`;
        this.events[id] = 'change';
        $(document).on(this.events[id], id, (eventObject) => {
            const element = $(eventObject.target);
     ...
     * @param id id for html
     */
    setChangeEventForScheduler(host, id) {
        id = `#${id}`;
        this.events[id] = 'change';
        $(document).on(this.events[id], id, (eventObject) => {
            const newData = $(eventObject.target).val();
     ...
     * @param prop
     */
    createButtonEvent(object, props) {
        const html = [];
        let title;
        props.forEach(prop => {
        ...
        if (prop.readonly && prop.readonly(this.tree, object)) {
            ...
            switch (prop.type) {
                ...
                case 'boolean':
                    ...
                    break;
                case 'host':
                    const hosts = this.hostInfos.map(host => {
                    ...
                    break;
                case 'scheduler':
                    const schedulers = SwfJobScheduler.schedulers().map(scheduler => {
    ...
    createPropertyHtml() {
        ...
        const createSubTitle = (object, prop) => {
            if (prop.button) {
