...
describe("test about cleaning functionality", function (){
  ...
  beforeEach(async()=>{
    ...
    await fs.outputFile(path.join(projectRootDir, "wf1", "task1", "run.sh"), "#!/bin/bash\npwd |tee result\n");
    await fs.outputFile(path.join(projectRootDir, "wf1", "wf2", "task2", "run.sh"), "#!/bin/bash\npwd |tee result\n");
  ...
  it("should clean only under wf1", async()=>{
    ...
    expect(path.join(projectRootDir, "task0", "result")).to.be.a.file().with.content(path.join(projectRootDir, "task0")+"\n");
