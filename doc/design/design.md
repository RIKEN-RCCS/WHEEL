...
index           | string       | コンポーネントを識別する一意な文字列
description     | string       | ユーザが指定するコンポーネントの説明文
previous        | number[]     | 先行コンポーネントのindex
next            | number[]     | 後続コンポーネントのindex
...
src         | string[] | 
src.srcNode | string   | 送信元ノードのindex
...
dst         | string[] | 
dst.dstNode | string   | 送信先ノードのindex
